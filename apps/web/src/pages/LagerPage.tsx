...
export default function LagerPage() {
  ...
  const canEdit = hasRole(user, ['PATRON', 'DON', 'CAPO', 'ADMIN', 'LOGISTICA'])
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="relative overflow-hidden bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 border-orange-500/30">
          ...
          <CardContent className="p-5">
            ...
            <p className="text-zinc-400 text-sm">Gesamte Artikel</p>
            <p className="text-2xl font-bold text-orange-400">{itemsData?.pagination?.total || 0}</p>
          </CardContent>
        </Card>
        ...
        <Card className="relative overflow-hidden bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 border-orange-500/20">
          ...
          <CardContent className="p-5">
            ...
            <p className="text-zinc-400 text-sm">Kategorien</p>
            <p className="text-2xl font-bold text-white">{categories?.length || 0}</p>
          </CardContent>
        </Card>
        
        <Card className={`relative overflow-hidden bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 ${criticalCount > 0 ? 'border-red-500/30' : 'border-orange-500/20'}`}>
          <div className={`absolute top-0 right-0 w-16 h-16 ${criticalCount > 0 ? 'bg-red-500/10' : 'bg-orange-500/10'} rounded-full blur-2xl pointer-events-none`} />
          <CardContent className="p-5">
            <div className="flex items-center gap-3 mb-2">
              <div className={`p-2 ${criticalCount > 0 ? 'bg-red-500/20' : 'bg-orange-500/20'} rounded-lg`}>
                <AlertTriangle className={`h-5 w-5 ${criticalCount > 0 ? 'text-red-400' : 'text-zinc-400'}`} />
              </div>
            </div>
            <p className="text-zinc-400 text-sm">Kritisch</p>
        ...
        <Card className="relative overflow-hidden bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 border-orange-500/20">
          ...
          <CardContent className="p-5">
            ...
            <p className="text-zinc-400 text-sm">Verfügbar</p>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 bg-zinc-800/50 border-zinc-700 focus:border-orange-500 h-11"
                />
              </div>
            </div>
            
            <div className="relative">
              <Filter className="absolute left-3 top-1/2 transform -translate-y-1/2 text-zinc-400 h-4 w-4 pointer-events-none" />
              <select
                value={selectedCategory}
                onChange={(e) => setSelectedCategory(e.target.value)}
                className="pl-10 pr-4 h-11 bg-zinc-800/50 border border-zinc-700 rounded-lg text-white focus:border-orange-500 focus:outline-none appearance-none cursor-pointer min-w-[180px]"
              >
            ...
            <Button
              ...
              className={showCriticalOnly 
                ...
                : 'border-zinc-700 hover:bg-zinc-800 h-11'
              }
            >
              <AlertTriangle className="mr-2 h-4 w-4" />
              Nur kritische
          ...
          {isLoading ? (
            <div className="flex justify-center py-16">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-zinc-400">Lade Artikel...</p>
              </div>
            </div>
          ) : itemsData?.items?.length === 0 ? (
            <div className="py-16 text-center">
              <Package className="h-16 w-16 mx-auto text-zinc-600 mb-4" />
              <p className="text-zinc-400 text-lg">Keine Artikel gefunden</p>
              <p className="text-zinc-500 text-sm mt-1">Versuche einen anderen Suchbegriff oder Filter</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-800 bg-zinc-800/30">
                    <th className="text-left py-4 px-6 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Artikel</th>
                    <th className="text-left py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Kategorie</th>
                    <th className="text-center py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Bestand</th>
                    <th className="text-center py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Verfügbar</th>
                    <th className="text-center py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Reserviert</th>
                  ...
                  {itemsData?.items?.map((item: any) => (
                    ...
                    >
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className={`w-2 h-2 rounded-full ${
                            item.isCritical ? 'bg-red-500 animate-pulse' : item.availableStock > 0 ? 'bg-green-500' : 'bg-zinc-500'
                          }`} />
                      ...
                      <td className="py-4 px-4 text-center">
                        ...
                        {item.minStock > 0 && (
                          <span className="text-zinc-500 text-xs ml-1">/ min {item.minStock}</span>
                        )}
                      </td>
                      <td className="py-4 px-4 text-center">
                      ...
                      <td className="py-4 px-4 text-center">
                        <span className={`font-semibold text-lg ${
                          item.reservedStock > 0 ? 'text-orange-400' : 'text-zinc-500'
                        }`}>
                          {item.reservedStock}
                        </span>
                      </td>
