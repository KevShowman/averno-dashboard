...
export default function LagerMovementsPage() {
  ...
  const getMovementTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'ADJUST': return 'bg-orange-500/20 text-orange-300 border-orange-500/30'
      case 'RESERVE': return 'bg-orange-500/20 text-orange-300 border-orange-500/30'
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 border border-orange-500/20 p-6">
        ...
        <div className="relative flex items-center justify-between">
          ...
          {pendingCount > 0 && (
            <Badge className="bg-orange-500/20 text-orange-300 border-orange-500/30 px-4 py-2 text-base animate-pulse">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card className="relative overflow-hidden bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 border-orange-500/20">
          <CardContent className="p-5">
            ...
            <p className="text-orange-300/70 text-sm">Korrekturen</p>
            <p className="text-2xl font-bold text-white">
          ...
          {isLoading ? (
            <div className="flex justify-center py-16">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-zinc-400">Lade Lagerbewegungen...</p>
              </div>
            </div>
          ) : (!pendingMovements || pendingMovements.length === 0) ? (
            <div className="py-16 text-center">
              <CheckCircle className="h-16 w-16 mx-auto text-green-500/50 mb-4" />
              <p className="text-zinc-400 text-lg">Keine ausstehenden Lagerbewegungen</p>
              <p className="text-zinc-500 text-sm mt-1">Alle Bewegungen wurden bereits bearbeitet</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-800 bg-zinc-800/30">
                    ...
                    <th className="text-center py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Typ</th>
                    <th className="text-center py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Menge</th>
                    <th className="text-center py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Von → Nach</th>
                    ...
                    <tr key={movement.id} className="group hover:bg-orange-950/20 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-zinc-500">{movement.item.category.name}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-4 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <span className="text-zinc-400">{movement.item.currentStock}</span>
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-zinc-300 text-sm">{getDisplayName(movement.createdBy)}</span>
                        </div>
                      </td>
                      <td className="py-4 px-4 max-w-xs">
                        <span className="text-zinc-400 text-sm truncate block">{movement.note || '-'}</span>
                      </td>
                      <td className="py-4 px-4">
                        <span className="text-zinc-400 text-sm">{formatDate(movement.createdAt)}</span>
                      </td>
                      {canApprove && (
                        <td className="py-4 px-6">
