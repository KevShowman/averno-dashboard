    ...
    return (
      <div className="p-6 flex items-center justify-center h-64">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-zinc-400">Lade Wochenabgabe-Daten...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <Card className="relative overflow-hidden bg-gradient-to-br from-zinc-800/80 to-zinc-900/80 border-orange-500/30">
            <CardContent className="p-5">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-orange-500/20 rounded-lg">
                  <Hourglass className="h-5 w-5 text-orange-400" />
                </div>
              </div>
              <p className="text-orange-300/70 text-sm">Ausstehend</p>
          ...
          <Card className={`relative overflow-hidden ${stats.overdue > 0 ? 'bg-gradient-to-br from-red-900/30 to-rose-900/20 border-red-500/30' : 'bg-gradient-to-br from-zinc-800/50 to-zinc-900/50 border-zinc-700'}`}>
            <CardContent className="p-5">
              <div className="flex items-center gap-3 mb-2">
                <div className={`p-2 ${stats.overdue > 0 ? 'bg-red-500/20' : 'bg-zinc-500/20'} rounded-lg`}>
                  <PackageX className={`h-5 w-5 ${stats.overdue > 0 ? 'text-red-400' : 'text-zinc-400'}`} />
                </div>
              </div>
              <p className={`${stats.overdue > 0 ? 'text-red-300/70' : 'text-zinc-400'} text-sm`}>Überfällig</p>
          ...
          <CardContent className="p-0">
            {exclusions.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-zinc-400">Keine aktiven Ausschlüsse</p>
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-zinc-800 bg-zinc-800/30">
                      ...
                      <th className="text-left py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Von</th>
                      <th className="text-left py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Bis</th>
                  ...
                  <tbody className="divide-y divide-zinc-800/50">
                    {exclusions.map((exclusion: WeeklyDeliveryExclusion) => (
                      <tr key={exclusion.id} className="group hover:bg-zinc-800/30 transition-colors">
                        <td className="py-4 px-6">
                          <span className="font-medium text-white">{formatUserName(exclusion.user)}</span>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-zinc-300">{exclusion.reason}</span>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-zinc-400">{formatDate(exclusion.startDate)}</span>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-zinc-400">{exclusion.endDate ? formatDate(exclusion.endDate) : 'Unbegrenzt'}</span>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-zinc-400">{exclusion.createdBy.username}</span>
                        </td>
                        {isLeadership && (
          ...
          <CardContent className="p-0">
            {currentWeekDeliveries.length === 0 ? (
              <div className="py-16 text-center">
                ...
                <p className="text-zinc-400">Keine Abgaben für diese Woche</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-zinc-800 bg-zinc-800/30">
                      <th className="text-left py-4 px-6 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Benutzer</th>
                      <th className="text-left py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Woche</th>
                      <th className="text-right py-4 px-4 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Bezahlt</th>
                  ...
                  <tbody className="divide-y divide-zinc-800/50">
                    {currentWeekDeliveries.map((delivery: WeeklyDelivery) => (
                      <tr key={delivery.id} className="group hover:bg-zinc-800/30 transition-colors">
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            <div className={`w-2 h-2 rounded-full ${
                              ...
                              delivery.status === 'OVERDUE' ? 'bg-red-500' : 'bg-orange-500'
                            }`} />
                        ...
                        <td className="py-4 px-4 text-right">
                          {delivery.paidAmount && delivery.paidAmount > 0 && (
                            <span className="text-orange-400 font-medium">{delivery.paidAmount} Pakete</span>
                          )}
                        ...
                        <td className="py-4 px-4 text-center">
                          {delivery.isAbgemeldet ? (
                            ...
                            <Badge className="bg-orange-500/20 text-orange-300 border-orange-500/30">Ausstehend</Badge>
                          ...
                          {(delivery.status === 'PENDING' || delivery.status === 'PARTIALLY_PAID') && 
                           ...
                           (canPayForOthers || delivery.userId === user?.id) && (
                            <Button
                              ...
                              className="opacity-0 group-hover:opacity-100 transition-opacity border-orange-500/30 text-orange-400 hover:bg-orange-500/10"
                            >
      ...
      {archives && archives.length > 0 && (
        <Card className="bg-zinc-900/50 border-zinc-800 overflow-hidden">
          ...
          <CardContent className="p-6">
            {loadingArchives ? (
              <div className="text-center py-8 text-zinc-400">Lade Archive...</div>
            ) : (
              <div className="space-y-4">
                {archives.map((archive: any) => (
                  <div key={archive.id} className="bg-zinc-800/30 border border-zinc-700/50 rounded-xl p-5">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                      ...
                      <div className="text-right">
                        <p className="text-xs text-zinc-500">Archiviert von</p>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                      <div className="bg-zinc-900/50 rounded-lg p-3 text-center">
                        ...
                        <p className="text-xs text-zinc-400">Gesamt</p>
                      </div>
                      <div className="bg-zinc-900/50 rounded-lg p-3 text-center">
                        <p className="text-xl font-bold text-green-400">{archive.paidDeliveries}</p>
                        <p className="text-xs text-zinc-400">Bezahlt</p>
                      </div>
                      <div className="bg-zinc-900/50 rounded-lg p-3 text-center">
                        <p className="text-xl font-bold text-red-400">{archive.overdueDeliveries}</p>
                        <p className="text-xs text-zinc-400">Überfällig</p>
                      </div>
                      <div className="bg-zinc-900/50 rounded-lg p-3 text-center">
                        <p className="text-xl font-bold text-orange-400">{archive.pendingDeliveries}</p>
                        <p className="text-xs text-zinc-400">Ausstehend</p>
                      </div>
                      <div className="bg-zinc-900/50 rounded-lg p-3 text-center">
                        <p className="text-xl font-bold text-orange-400">{archive.paidPackages}</p>
                      ...
                      <div className="bg-zinc-900/50 rounded-lg p-3 text-center">
                        ...
                        <p className="text-xs text-zinc-400">Geld</p>
                      </div>
                    </div>
                  </div>
                ))}
